...
This launch file creates a 5-minute long scenario with waypoint-based zigzag trajectory:
...
Key Features:
  - 5 phases with zigzag trajectory
  - Altitude changes (1.0m → 3.0m → 1.5m → 2.5m → 2.0m)
  - Waypoint-based motion (not static targets)
...
def generate_launch_description():
    """Generate launch description for long scenario demonstration"""
    ...
    # IMPORTANT: Using LONG SCENARIO config files with waypoints!

    # Formation config files for LONG SCENARIO
    formation_config_group0 = os.path.join(
        pkg_formation_coordinator,
        'config',
        'long_scenario_group0_fuzzy.yaml'  # ← WAYPOINT-BASED CONFIG
    )
    formation_config_group1 = os.path.join(
        pkg_formation_coordinator,
        'config',
        'long_scenario_group1_pd.yaml'  # ← WAYPOINT-BASED CONFIG
    )
    formation_config_group2 = os.path.join(
        pkg_formation_coordinator,
        'config',
        'long_scenario_group2_pid.yaml'  # ← WAYPOINT-BASED CONFIG
    )

    # Group 0 Formation Coordinator (PID+Fuzzy) - LONG SCENARIO
    ...
    # Group 1 Formation Coordinator (PD) - LONG SCENARIO
    ...
    # Group 2 Formation Coordinator (PID) - LONG SCENARIO
    ...
    return LaunchDescription([
        ...
        *agent_nodes,
        ...
        # Formation coordinators (3 groups) - LONG SCENARIO with waypoints
