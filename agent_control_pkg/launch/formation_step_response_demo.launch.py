#!/usr/bin/env python3
"""
Formation Step Response Demo - Launch File

Fast step response analysis demonstration with 3 formation groups:
  - Group 0: 3x PID+Fuzzy controllers (agent_0,1,2)
  - Group 1: 3x PD controllers (agent_3,4,5)
  - Group 2: 3x PID controllers (agent_6,7,8)

Each group performs a fast step response test (60 seconds):
  - Initial position: X=-15m, Y varies by group
  - Step target: X=5m (20m step forward)
  - Velocity: 2 m/s (fast but stable)
  - Duration: 60s total (includes settling time)

This demo is designed for step response analysis (MATLAB stepinfo() equivalent):
  - Rise time (10%-90%)
  - Overshoot (%)
...
Usage:
    ...
    ros2 launch agent_control_pkg formation_step_response_demo.launch.py
...
def generate_launch_description():
    """Generate launch description for step response demonstration"""
    ...
    # **UPDATED: Using step response config files (fast motion, no waypoints)**

    # Formation config files - STEP RESPONSE
    formation_config_group0 = os.path.join(
        pkg_formation_coordinator,
        'config',
        'step_response_group0_fuzzy.yaml'  # <-- STEP RESPONSE CONFIG
    )
    formation_config_group1 = os.path.join(
        pkg_formation_coordinator,
        'config',
        'step_response_group1_pd.yaml'     # <-- STEP RESPONSE CONFIG
    )
    formation_config_group2 = os.path.join(
        pkg_formation_coordinator,
        'config',
        'step_response_group2_pid.yaml'    # <-- STEP RESPONSE CONFIG
    )
    ...
    return LaunchDescription([
        ...
        *agent_nodes,
        ...
        # Formation coordinators (3 groups) - STEP RESPONSE
