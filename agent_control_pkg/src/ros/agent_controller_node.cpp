...
: rclcpp::Node("agent_controller_node", options)
{
  ...
  // Controller parameters publisher (always enabled for monitoring dashboard)
  ...
  // Publish initial controller parameters
  publishControllerParams();
...
void AgentControllerNode::publishControllerParams()
{
  ...
  // Timing
  msg.control_frequency_hz = (loop_period_ > 0.0) ? (1.0 / loop_period_) : 200.0;
